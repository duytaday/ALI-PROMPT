...
- Lead capture lưu trong Cloudflare D1, có consent, validation và gom lần gửi lặp theo cùng liên hệ + nhu cầu.
- Trang `/admin` chỉ dành cho chủ sở hữu để xem và xuất lead an toàn.
...
`npm test` kiểm cả AI contract, production build và contract test của web.
...
- `docs/operations-runbook.md`: quy trình theo dõi lead và vận hành workshop thủ công.
- `docs/release-readiness.md`: phần đã chạy, phần mới là blueprint và các cổng trước khi mở public.
